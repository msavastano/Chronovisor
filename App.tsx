...
import { executeTimeTravel, lookupHistoricalEvent } from './services/geminiService';

function App() {
  const [selectedCoordinates, setSelectedCoordinates] = useState<Coordinates | null>(null);
  const [apiKey, setApiKey] = useState<string>(() => localStorage.getItem('chronovisor_api_key') || '');
  const [isMockMode, setIsMockMode] = useState<boolean>(() => !localStorage.getItem('chronovisor_api_key'));

  const handleSetApiKey = (key: string) => {
    setApiKey(key);
    if (key.trim()) {
      localStorage.setItem('chronovisor_api_key', key.trim());
      setIsMockMode(false);
    } else {
      localStorage.removeItem('chronovisor_api_key');
      setIsMockMode(true);
    }
  };
  ...
  const handleEventLookup = async (query: string) => {
    setError(null);
    try {
      const data = await lookupHistoricalEvent(query, isMockMode, apiKey);
      if (data) {
    ...
    <div className="min-h-screen bg-black text-cyan-500 holo-grid overflow-hidden flex flex-col font-mono">
      ...
      <main className="flex-1 p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-y-auto">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6 h-full">
           <ViewScreen result={result} loading={isTraveling} error={error} onDismissError={() => setError(null)} />
